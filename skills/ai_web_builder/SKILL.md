...
The manifest is rebuilt from disk by the entry script (`scripts/run-ai-briefing.sh` → `commit_and_push` in `scripts/briefing-common.sh` → `scripts/rebuild_manifest.py`) as a post-step after the parallel builders finish. The manifest is a pure derived index, so regenerating it from disk always yields the correct merged result even when the GitHub and US-stocks briefings push concurrently (a manifest rebase conflict is auto-resolved by regenerating). Simply write the new `web/AI/<date>/data.json` and the rebuild step will pick it up automatically.
